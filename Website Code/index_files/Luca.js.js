...
/*class path{
    ...
    constructor(xIN,yIN,blocked,ancestor=false){
        this.coord={x:xIN, y:yIN};
        this.blocked=blocked;
        if(ancestor!=false){
            this.had=[this];//set had:the routes to come here
        }else{//add this path to had
            this.had=this.ancestor.had.slice();
            this.had.push(this);
        }
    }
    determineDistance(to){
        this.distance=getRoute(coord, to.coord);
        return this.distance;
    }

    
    //Geef de kruizing een code die anders is dan die van de aanliggende kruizingen
    setCode(){
        this.code=this.y%2+this.x%2==0;
    }
